...
| `src/calendly/` | Recordatorios precall a closers. **`programs.js`** = registro `PROGRAMS` de primera clase (label, company, connection, eventType, pitch, materiales) — fuente única de la que se derivan los mapas de copy/ET. **`accounts.js`** = registro de **conexiones de Calendly** (token, org, dry-run; los eventTypes se derivan de `programs.js`). **`closers.js`** = roster keyeado por **persona con identidades** (una por conexión); deriva `CLOSERS`/`CLOSER_LIDS`. `workLid` PINNEA el destino de los pushes; `extraJids` agrega una COPIA a un segundo aparato (§18.BA) y **saltea el gate anti-ban** → solo con tráfico entrante probado. **`lead-form.js`** = el formulario del anuncio como 2da fuente del teléfono del lead, SOLO para las conexiones que declaran `leadForm` (las dos de Retia, que no tienen HubSpot). Si el número difiere del de Calendly, los pushes que llevan el número (0, 1, 2, 3 y la reagenda correctiva) van con los dos, rotulados por lead y por fuente; si Calendly no trae número, lo rescata ([ADR 0003](docs/adr/0003-segunda-fuente-del-telefono-del-lead.md) + §18.CA). Modelo empresa/programa/closer: [ADR 0001](docs/adr/0001-modelo-empresa-programa-closer.md) + glosario [docs/agents/context.md](docs/agents/context.md). |
| `src/sheets/` | Lector read-only de Google Sheets (service account, sin `googleapis`). Dos consumidores: el reporte diario de leads y, vía `fetchSheetValues`, el formulario del anuncio de Retia (`calendly/lead-form.js`) |
...
- **En Windows `npm test` da ~64 fallos que NO son reales:** `better-sqlite3` no tiene binario
  ...
  *"Could not locate the bindings file"*. El baseline REAL se saca en Linux:
  ...
  Baseline al **2026-09-18: 1234 tests, 1232 verdes, 2 rojos conocidos** (`call con TODOS sus
